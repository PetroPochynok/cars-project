...
<table class="table" style="margin-left: auto; margin-right: auto; width: 80%;" border="1">
    ...
    <c:forEach var="tempCar" items="${cars}">
        ...
        <c:url var="updateLink" value="/cars/showFormForUpdateUsedCar">
            <c:param name="carId" value="${tempCar.id}"/>
        </c:url>

        <c:url var="deleteLink" value="/cars/deleteUsedCar">
            <c:param name="carId" value="${tempCar.id}"/>
        </c:url>

        <c:url var="buyUsedCar" value="/cars/buyUsedCar">
            <c:param name="carId" value="${tempCar.id}"/>
        </c:url>
        ...
        <c:url var="rentLink" value="/cars/showFormForRentCar">
            <c:param name="carId" value="${tempCar.id}"/>
        </c:url>
        ...
        <tr>
            ...
            <td style="text-align:center"><c:choose>
                <c:when test="${tempCar.status == 'NEW CAR ON SALE'}">New Car</c:when>
                <c:when test="${tempCar.status == 'USED CAR ON SALE'}">Used Car</c:when>
                <c:when test="${tempCar.status == 'CAR TO RENT ON SALE'}">Car to Rent</c:when>
            </c:choose></td>
            <td>${tempCar.mileage}</td>
            <td style="text-align:center">
                <c:choose>
                    <c:when test="${tempCar.status == 'CAR TO RENT ON SALE'}">
                        -
                    </c:when>
                    <c:otherwise>
                        ${tempCar.price}$
            ...
            <td>
                <c:choose>
                    ...
                    <c:when test="${tempCar.status == 'NEW CAR ON SALE'}">
                        <a href="${buyNewCar}" class="btn btn-success btn-sm">Buy Car</a>
                    </c:when>

                    <c:when test="${tempCar.status == 'USED CAR ON SALE'}">
                        <a href="${buyUsedCar}" class="btn btn-success btn-sm">Buy Car</a>
                    </c:when>

                    <c:when test="${tempCar.status == 'CAR TO RENT ON SALE'}">
                        <a href="${rentLink}" class="btn btn-primary btn-sm">Rent</a>
                    </c:when>
                </c:choose>
